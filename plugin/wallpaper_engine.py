...
PATH = r'C:\Steam\steamapps\common\wallpaper_engine'
...
def get_profiles(wallpaper_engine_path, exe=EXE32):
    ...
    for key in config.keys():
        if key == '?installdirectory' or key == '_installdirectory':
            continue
        else:
            user = config[key]
    for profile in user['general'].get('profiles', []):
        profiles.append(Profile(profile, wallpaper_engine_path))
    return profiles
...
class WallpaperEngine(object):
    ...
    def __init__(self, wallpaper_engine_dir, exe=EXE32):
        ...
        self.profiles = get_profiles(self.wallpaper_engine_dir, self.exe)
